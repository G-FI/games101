...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        //TODO: 为什么设置view space normal，以及它为什么这样计算
        ...
        for(int x = min_x; x < max_x; ++x){
            ...
            if(insideTriangle(x+0.5, y+0.5, t.v)){
                ...
                if(zp <= depth_buf[ind] + EPSILON){
                    ...
                    auto interpolated_tc = interpolate(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1], t.tex_coords[2],1.f);
                    auto interpolated_shadingcoords = interpolate(alpha, beta, gamma, view_pos[0], view_pos[1], view_pos[2],1.f);//TODO 要这个干嘛？
                    fragment_shader_payload frag_payload(interpolated_color, interpolated_normal.normalized(), interpolated_tc, texture? &*texture: nullptr);
                    frag_payload.view_pos = interpolated_shadingcoords;
                    auto color = fragment_shader(frag_payload);
                    set_pixel({x, y}, color);
                }
            }
        }
